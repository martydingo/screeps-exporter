...
class Memory:
    ...
    def pollRoomEnergyMetrics(self):
        for roomName in self.roomMemory:
            try:
                energyData = self.roomMemory[roomName]["energy"]
                self.metrics["energy"]["amount"].labels(room=roomName).observe(
                    energyData["amount"]
                )

                self.metrics["energy"]["capacity"].labels(room=roomName).observe(
                    energyData["capacity"]
                )
        ...
        for roomName in self.roomMemory:
            try:
                ...
                for sourceId in sourceData:
                    self.metrics["sources"]["amount"].labels(
                        room=roomName, source=sourceId
                    ).observe(sourceData[sourceId]["energy"]["amount"])
                    self.metrics["sources"]["capacity"].labels(
                        room=roomName, source=sourceId
                    ).observe(sourceData[sourceId]["energy"]["capacity"])
                    self.metrics["sources"]["regeneration"].labels(
                        room=roomName, source=sourceId
                    ).observe(sourceData[sourceId]["regeneration"])
